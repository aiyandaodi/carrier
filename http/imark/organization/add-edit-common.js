 ...
 **************************************************/
//=====================================================Parameter=============================
var TAG_OBJECT = "Organization";    // 默认标签对象名
...
$(document).ready(function(){
	...
	$('#tag-modal-dialog').on('hidden.bs.modal', function (e) {
	  $("#org-common-tags-ul").append(getAddCustomTagHTML());
	})	
})
//-----------------------------------------------------时间格式转化------------------------------------------------
//将long行时间转化成的时分秒对象
...
function getTags(subjectTags) {
	...
	var callback = {
		...
		error: function (errorMsg) {
			$("#org-common-tags-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#org-common-tags-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}
...
function showSubjectTags(tags, subjectTags) {
	if (!tek.type.isArray(tags) || tags.length <= 0){
		if(!tek.tag.isCanCreate(myId)){
				$("#organization_tags-form-group").empty();
			}
		return;
	}
...
function removeSubjectTag(targetId) {
	...
	$("#org-common-tags-ul").html('');
	$("#org-common-tags-ul").append($(tagsStr)).append(getAddCustomTagHTML());
