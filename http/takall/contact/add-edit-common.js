...
var COLOR_OBJECT = "Contact"	 //默认颜色对象名
...
$(document).ready(function(){
	...
	$('#tag-modal-dialog').on('hidden.bs.modal', function (e) {
	  $("#contact-common-tags-ul").append(getAddCustomTagHTML());
	})	
})
//-----------------------------------------------------时间格式转化------------------------------------------------
//将long行时间转化成的时分秒对象
...
function getTags(subjectTags) {
	if (!$("#contact-common-tags-ul").length)
		return;
	...
	var callback = {
		success: function (tags) {
			$("#contact-common-tags-ul").empty();	//清空
			//显示标签列表
		...
		error: function (errorMsg) {
			$("#contact-common-tags-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#contact-common-tags-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}
...
function showSubjectTag(id, name, type, isChecked) {
	...
	var html = "<li>"
		+ "<input name='contact_tags' type='checkbox' id='tag_" + id + "' value='" + tek.dataUtility.stringToHTML(name)
	...
	$("#org-common-tags-ul").append(html);
}
...
function removeSubjectTag(targetId) {
	...
	$("#contact-common-tags-ul").html('');
	$("#contact-common-tags-ul").append($(tagsStr)).append(getAddCustomTagHTML());
...
function getColors(contactColors) {
	if (!$("#contact-common-color-ul").length)
		return;
	...
	var callback = {
		success: function (colors) {
			$("#contact-common-color-ul").empty();	//清空
			//显示标签列表
			showSubjectColors(colors, contactColors);
			//检测是否可以添加自定义标签
			/*if (tek.color.isCanCreate(myId))
				$("#works-common-color-ul").append(getAddCustomTagHTML());*/
		},
		error: function (errorMsg) {
			$("#contact-common-color-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#contact-common-color-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}

//显示颜色列表
function showSubjectColors(colors, contactColors) {
	if (!tek.type.isArray(colors) || colors.length <= 0){
		if(!tek.color.isCanCreate(myId)){
				$("#contact_color-form-group").empty();
			}
		return;
	}

	var subjectTagArr = subjectTagsTosubjectTagArr(contactColors);	//转换为数组
	selected_tags_sum = subjectTagArr.length;  //保存已选中的标签数
...
function showSubjectColor(id, name, type, value, isChecked) {
	...
	$("#contact-common-color-ul").append(html);
}
