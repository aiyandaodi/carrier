...
//发表动态
function submitAdd(){
	var setting = {operateType: "发表动态信息！"}
	var params = {}
	params["objectName"] = "Interaction";
	params["action"] = "addInfo";
	params["interaction_remark"] = $("#interaction_remark").val();
	
	var checkVal=$("#change-interaction-read").find("option:selected").val();
	if(checkVal){
		params["interaction_read"] = checkVal;
	}
...
//获取动态信息
function getInteractionList(skip){
	var target = document.getElementById('interaction_list');
    if(!target){
		return;
	}
	
	var setting = {async:false,operateType: "获取动态信息"};
	...
	var callback = {
        ...
        success: function (data) {
            ...
            if (record) {
                ...
                for(var i in record){
					showInteractionList(record[i], data.right);
				}
                ...
                $("#interaction_list").html("<div class='col-md-12 col-sm-12 center'>没有动态信息！</div>");
            }
        },
        error: function (data, errorMsg) {
...
//显示动态信息
function showInteractionList(record, right){
	if(!record){
		return;
	}
	var html = '';
	...
	if(ICON){
		html += '<a href="#" class="ui-icon imgPC"><img class="img-responsive pull-left user-icon-' + record.id + '" src="'+ICON+'"></a>';
	}
	html += '<div class="ui-rcontent">'
		+ '<div class="pull-right">';

	/*html += '<span class="dropdown">'
		+ '<a href="javascript:void(0)" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-gear"></i></a>'
	...
	if(record.interaction_owner){
		...
		html += '<h4>';
		if(ICON){
			html += '<a class="ui-icon imgMobile"><img class="img-responsive pull-left user-icon-' + record.id + '" src="'+ICON+'"></a>'
			
		}	
		html += '<a class="user-name">' + tek.dataUtility.stringToHTML(field.show || '') + '</a></h4>';
	}
	if(record.interaction_remark){
		field = record.interaction_remark;
		html += '<p style="cursor:pointer;" id="content-' + record.id + '"  onclick=readInteraction("'+record.id+'")>' + tek.dataUtility.stringToHTML(field.show || '') + '</p>';
	}
	...
	html += '<div class="ui-btns">'
		...
		+ '<a href="javascript:void(0)" onClick=javascript:openComment(this,"'+record.id+'","'+record.interaction_owner.show+'")><i class="fa fa-commenting-o"></i>&nbsp;评论<span></span></a>&nbsp;&nbsp;'
	...
	// var html1 = getReplyList(record.id);
	...
	// 	+ '</div>'
		...
		+ '<form id="reply-form-'+record.id+'">'
			+ '<div class="input-group">'
//				+ '<span class="input-group-addon"><i class="fa fa-user-circle"></i></span>'
				+ '<input type="text" class="form-control" id="object_reply_remark-'+record.id+'" name="object_reply_remark" placeholder="写下你的评论...">'
				+ '<div class="input-group-btn">'
					+ '<button class="btn btn-primary" type="button" onclick=addReply("'+record.id+'")>评论</button>'
				+ '</div>'
			+ '</div>'
		+ '</form>'
		+ '</div>'*/
...
//读取动态信息
function readInteraction(id){
	var url = tek.common.getRootPath() + 'http/takall/interaction/read.html?interaction_id='+id;
	window.open(url);
}
...
function showReplyList(record,linkId,name, wrapDiv){
	...
	var html = '';
	var field;
	html += '<div class=" user-comment" id="user-comment-' + record.id + '">';
	...
	// if(record.object_reply_name){
	// 	field = record.object_reply_name;
	// 	html += '<h4 class="">'+tek.dataUtility.stringToHTML(field.show || '')+'</h4>'
	// }
	...
	html += '<form id="reply-form-'+record.id+'">'
		...
		+ '<input type="hidden" value="'+myName+'回复'+record.object_reply_user.show+'" class="form-control" id="object_reply_name" name="object_reply_name">'
		+ '<div class="input-group-btn">'
