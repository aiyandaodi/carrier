 ...
 **************************************************/
//=====================================================Parameter=============================
var TAG_OBJECT = "FavoriteSet";    // 默认标签对象名
var COLOR_OBJECT = "FavoriteSet"	 //默认颜色对象名
...
$(document).ready(function(){
	...
	$('#tag-modal-dialog').on('hidden.bs.modal', function (e) {
	  $("#favorite_set-common-tags-ul").append(getAddCustomTagHTML());
	})	
})
//-----------------------------------------------------时间格式转化------------------------------------------------
//将long行时间转化成的时分秒对象
...
function getTags(subjectTags) {
	if (!$("#favorite_set-common-tags-ul").length)
		return;
	...
	var callback = {
		success: function (tags) {
			$("#favorite_set-common-tags-ul").empty();	//清空
			//显示标签列表
		...
		error: function (errorMsg) {
			$("#favorite_set-common-tags-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#favorite_set-common-tags-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}
...
function showSubjectTags(tags, subjectTags) {
	if (!tek.type.isArray(tags) || tags.length <= 0){
		if(!tek.tag.isCanCreate(myId)){
				$("#favorite_set_tag-form-group").empty();
			}
		return;
	}
...
function showSubjectTag(id, name, type, isChecked) {
	...
	var html = "<li>"
		+ "<input name='favorite_set_tag' type='checkbox' id='tag_" + id + "' value='" + tek.dataUtility.stringToHTML(name)
	...
	$("#favorite_set-common-tags-ul").append(html);
}
...
function addSubjectTagBtn(targetId) {
	...
	$("#" + targetId + "-loading").html("<img src='../favorate/" + tek.common.getRootPath() + "http/images/waiting-small.gif' />");
...
function removeSubjectTag(targetId) {
	...
	for(var i = 0, len = nodes.length; i < len; i++){
		...
		tagsStr += '<li>';
		tagsStr += '<input name="favorite_set_tag" type="checkbox" id="tag_'+ nodeId +'" value="'+ tek.dataUtility.stringToHTML(strHtml) +'" onchange="countSelectedTagsTotal(this)">';
	...
	$("#favorite_set-common-tags-ul").html('');
	$("#favorite_set-common-tags-ul").append($(tagsStr)).append(getAddCustomTagHTML());
...
function removeSubjectTagBtn(id) {
	...
	tek.macCommon.waitDialogShow(null, "<img src='../favorate/" + tek.common.getRootPath() + "http/images/waiting-small.gif'/> 正在删除...");
...
function getColors(worksColors) {
	if (!$("#favoriteSet-common-color-ul").length)
		return;
	...
	var callback = {
		success: function (colors) {
			$("#favoriteSet-common-color-ul").empty();	//清空
			//显示标签列表
			showSubjectColors(colors, worksColors);
			//检测是否可以添加自定义标签
			// if (tek.color.isCanCreate(myId))
			// 	$("#favoriteSet-common-color-ul").append(getAddCustomTagHTML());
		},
		error: function (errorMsg) {
			$("#favoriteSet-common-color-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#favoriteSet-common-color-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}
...
function showSubjectColor(id, name, type, value, isChecked) {
	...
	$("#favoriteSet-common-color-ul").append(html);
}
...
// -------------------------------------------------------------------
// 鉴定业务范围相关的函数开始
function remarkStr(placeholder){
	var holder = placeholder || "";
	var add_input_str = "<input type='text' id='tianjia_yingyefanwei' onkeydown='inputing(event);' onkeyup='inputing(event);' placeholder='"+ holder +"'>";
	var add_btn_str = "<a id='add_one_range' onclick='addOneRange();'>添加</a>";

	var html = "";
	html += "<div class='col-xs-9'>";
	html += "<div>" + add_input_str + add_btn_str +"</div>";
	html += "<div id='show_remarks'></div>";
	html += "</div>";
	return html;
}
...
function inputing(event){
	...
	var check_input = new RangeInput();

	var check_value = check_input.getValue();
	var len = check_value.length;
	var last_letter = check_value[len - 1];

	// 如果是空格，逗号，<，>等则不可以输入
	var super_patter = /[\s<>,]/;

	var test_condition = super_patter.test(last_letter);

	if(test_condition){
		var pre_value = check_value.substring(0,len - 1);
		check_input.setValue(pre_value);
		return;
	}
...
function RangeInput(){
	this.ele = document.getElementById("tianjia_yingyefanwei");
...
function addOneRange(){
	var range_input = new RangeInput();
	var range_value = range_input.getValue();
	if(range_value.length === 0)return;
	range_input.setValue("");
	var range_wraper = document.getElementById("show_remarks");
	var a_range_wraper = getARangeDom(range_value);
	range_wraper.appendChild(a_range_wraper);
}

function getARangeDom(range_value){
	COMMON_INDEX++;

	var a_range_wraper = document.createElement("div");
	a_range_wraper.setAttribute("class","a_range_wraper");

	var value_span = document.createElement("span");
	value_span.innerHTML = range_value;
	value_span.setAttribute("class","range_content");

	var value_span1 = document.createElement("span");
	value_span1.innerHTML = "+";
	value_span1.style.transform = "rotate(45deg)";
	value_span1.setAttribute("class","delete_this_range");

	a_range_wraper.appendChild(value_span);
	a_range_wraper.appendChild(value_span1);

	RANGE_OBJ[COMMON_INDEX] = {
		ele : a_range_wraper,
		index : COMMON_INDEX,
		value : range_value
	}

	var temp_index = COMMON_INDEX;

	value_span1.addEventListener("click",function(event){
		a_range_wraper.outerHTML = null;
		var temp_index1 = temp_index;
		delete RANGE_OBJ[temp_index1];
	});

	return a_range_wraper;
}

function getRangeList(){
	var str = "";
	for(var name in RANGE_OBJ){
		str += RANGE_OBJ[name]["value"] + ",";
	}

	return str;
}

// organization_remark相关的函数结束