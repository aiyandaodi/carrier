...
{ 
  window.open("read.html?question_id="+id+"&refresh-opener=1");
}

//编辑用户
function editInfo(id)
{ 
  window.open("edit.html?question_id="+id+"&show-close=1&refresh-opener=1");
}
    ...
    success: function(data) {
        if(data){
          if (data.code==0){
			...
			tek.macCommon.waitDialogShow(null, data.code+" - "+data.message);
        } else
