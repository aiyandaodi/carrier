 ...
 **************************************************/
//=====================================================Parameter=============================
var TAG_OBJECT = "Profile";    // 默认标签对象名
var COLOR_OBJECT = "Profile"	 //默认颜色对象名
var groupId;    // 所属小组标识
...
$(document).ready(function(){
	...
	$('#tag-modal-dialog').on('hidden.bs.modal', function (e) {
	  $("#profile-common-tags-ul").append(getAddCustomTagHTML());
	})	
})
//-----------------------------------------------------时间格式转化------------------------------------------------
//将long行时间转化成的时分秒对象
...
function getTags(subjectTags) {
	if (!$("#profile-common-tags-ul").length)
		return;
	...
	var callback = {
		success: function (tags) {
			$("#profile-common-tags-ul").empty();	//清空
			//显示标签列表
		...
		error: function (errorMsg) {
			$("#profile-common-tags-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#profile-common-tags-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}
...
function showSubjectTag(id, name, type, isChecked) {
	...
	var html = "<li>"
		+ "<input name='profile_tag' type='checkbox' id='tag_" + id + "' value='" + tek.dataUtility.stringToHTML(name)
	...
	$("#profile-common-tags-ul").append(html);
}
...
function removeSubjectTag(targetId) {
	...
	for(var i = 0, len = nodes.length; i < len; i++){
		...
		tagsStr += '<li>';
		tagsStr += '<input name="profile_tag" type="checkbox" id="tag_'+ nodeId +'" value="'+ tek.dataUtility.stringToHTML(strHtml) +'" onchange="countSelectedTagsTotal(this)">';
	...
	$("#profile-common-tags-ul").html('');
	$("#profile-common-tags-ul").append($(tagsStr)).append(getAddCustomTagHTML());
...
function getColors(worksColors) {
	if (!$("#profile-common-color-ul").length)
		return;
	...
	var callback = {
		success: function (colors) {
			$("#profile-common-color-ul").empty();	//清空
			//显示标签列表
			showSubjectColors(colors, worksColors);
			//检测是否可以添加自定义标签
			// if (tek.color.isCanCreate(myId))
			// 	$("#profile-common-color-ul").append(getAddCustomTagHTML());
		},
		error: function (errorMsg) {
			$("#profile-common-color-ul").html("<div style='text-align:center;color:red;'>" + errorMsg + "</div>");
		}
	};
	...
	if (re.flag) {
		...
		$("#profile-common-color-ul").html("<div style='text-align:center;color:red;'>" + re.message + "</div>");
	}
}
...
function showSubjectColors(colors, worksColors) {
	if (!tek.type.isArray(colors) || colors.length <= 0){
		if(!tek.color.isCanCreate(myId)){
				$("#profile_color-form-group").empty();
			}
		return;
	}
...
function showSubjectColor(id, name, type, value, isChecked) {
	...
	$("#profile-common-color-ul").append(html);
}
