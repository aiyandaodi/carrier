...
function callSubServicePage(path,type){
	if(path){
		if(LATEST_CALL_PATH){
		...
		if(type){
			sessionStorage.setItem('openSubClass', type);
		}
		if(sessionStorage){
...
function initFn(){
	...
	if(sessionStorage){
		...
		var openClass = sessionStorage.getItem('openClass');
		var openSubClass = sessionStorage.getItem('openSubClass');
		var path = sessionStorage.getItem('path');
		if(openClass){
		  	showOpenClass(openClass)
		}
		if(openSubClass){
		  	showSubClass(openSubClass);
		}
		if(path){
			callSubServicePage(path);
		}
	}
}
//---------------------------------------------------------------------------------------------------
...
function showOpenClass(target) {
    ...
    // var sessionStorage = window.sessionStorage;
    if(sessionStorage){
    	sessionStorage.setItem('openClass', openClass);
    }
}
...
function showSubClass(target) {
    ...
    if(sessionStorage){
    	sessionStorage.setItem('openSubClass', openSubClass);
    }
}
