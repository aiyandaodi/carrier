...
// 跳转到ipass登录页面
function iLogin() {
	var msg = "正在跳转到登录页面，请稍后...";
	tek.macCommon.waitDialogShow("<font color='red'>请先登录</font>", msg, null, 2);
...
// 跳转到ipass注册页面
function ipassRegister() {
  //tek.common.callPage(tek.common.getRootPath()+"http/hdw.top/login/register.html", location.href);
  //tek.common.callPage("http://10.128.0.2:1999/http/ipass/login/register.html", location.href);
  
  var url=tek.common.getAuthUrl("register");
  //alert(url);
  if(url && url.length>0){
	  tek.common.callPage(url, location.href);
  }else{
	  tek.macCommon.waitDialogShow(null, "无法取得注册页面！");
  }
