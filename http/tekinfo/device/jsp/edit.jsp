...
<link href="../../../common/tek/css/edit.css" rel="stylesheet" type="text/css" media="screen" />
...
<%
	...
	long deviceId = DataUtility.StringToLong(parameters.getParameter("device_id"));    //对象标识
	...
	ObjectResult result = remoting.getEdit(token.getTokenCode(), clientIp, parameters.getParameters());
...
%>
  <!-- 修改框 -->
  <div id="info">
    <form action="editCheck.jsp" id="editForm" name="editForm" method="post" accept-charset="<%= charset %>">
<%
			// 字段
			...
			// 其他参数
			out.println(new StringBuilder("<input type='hidden' id='device_id' name='device_id' value='").append(deviceId).append("'/>"));
			...
			boolean refreshOpener = DataUtility.StringToBoolean(parameters.getParameter("refresh-opener"));    //是否刷新opener页
...
<%
		} // end if (records != null && records.length > 0 && records[0] != null) else
