...
var selected_objectName = null; //被选中对象名

//=====================================================Function===============================
//获得显示的字段
function editNew(option_id) {
    var params = {};
...
function showSelectOptionInfo(values) {
    ...
    for (var i = 0, len = values.length; i < len; i++) {
        ...
        if (!tek.type.isEmpty(value[0]) && !tek.type.isEmpty(value[1])) {
            var selected = (selected_objectName && selected_objectName == value[0]) ? 'selected' : '';
            html += "<option value='" + value[0] + "' " + selected + ">" + value[1] + " (" + value[0] + ")</option>";
        }
    }
...
tek.macEdit.appendCustomEditField = function (field, record) {
    ...
    if (fieldname == "option_objectName") {
        ...
        selected_objectName = field.value;
    } else {
