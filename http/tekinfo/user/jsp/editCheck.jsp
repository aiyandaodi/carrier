...
-->
<html xmlns="http://www.w3.org/1999/xhtml"><!-- #BeginTemplate "/Templates/editCheck.dwt.jsp" --><!-- DW6 -->
<head>
...
<%
	remoting = new UserRm();
	result = remoting.getEdit(token.getTokenCode(), clientIp, p);
%>
<!-- #EndEditable -->
...
<!-- 调用修改信息的方法 -->
<%
				result = remoting.setInfo(token.getTokenCode(), clientIp, parameters.getParameters(fields));
%>
<!-- #EndEditable -->
...
<!-- 操作成功的界面 -->
<%
		long userId = DataUtility.StringToLong(parameters.getParameter(User.FIELD_ID));    //对象标识
		if (userId != 0 && userId == my.getId()) {
	/*		String pwd = parameters.getParameter(User.FIELD_PASSWORD);
			if (pwd != null && !pwd.isEmpty()) {
				// 修改自己的密码，更新至attribute，用于其他webapp登录
				String algorithm = TekInfo.getInstance().getXMLProperties().get(
						TekInfoProperties.PREFIX_HOST + TekInfoProperties.SEP
								+ "WebService", "algorithm");
				String key = TekInfo.getInstance().getXMLProperties().get(
						TekInfoProperties.PREFIX_HOST + TekInfoProperties.SEP
								+ "WebService", "key");
				String secretPwd = null;
				try {
					secretPwd = AlgorithmFactory.Encode(algorithm, key, pwd);
				} catch (Exception e) {
					e.printStackTrace();
				}
