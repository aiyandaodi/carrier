...
参数：该页面使用参数。
-->
<html xmlns="http://www.w3.org/1999/xhtml"><!-- #BeginTemplate "/Templates/export.dwt.jsp" --><!-- DW6 -->
<head>
...
<!-- #EndEditable -->
<%
	Result result = remoting.getExportTempletNames(token.getTokenCode(), clientIp, parameters.getParameters());
...
%>
    <form action="exportCheck.jsp" id="exportForm" name="exportForm" method="post" accept-charset="<%= charset %>">
      <!-- 模板类型 -->
      <ul class='field'>
        <li class='title'>导出类型</li><br/>
        <li class='value'>
...
%>
      ...
      <!-- 是否包含BLOB数据 -->
      <ul class='field'>
        <li class='title'>包含BLOB数据</li><br/>
        <li class='value'>
          <input type="radio" id="withLocalFile" name="withLocalFile" value="1"/>是<br/>
          <input type="radio" id="withLocalFile" name="withLocalFile" value="0" checked="checked"/>否<br/>
        </li>
      </ul>
      <!-- 密码 -->
      <ul class='field' id='pass' name='pass'>
        ...
        <li class='value'>
          <input type="password" id="transfer-password" name="transfer-password"/>
        </li>
      </ul>
...
%>
      <!-- 按钮 -->
      <ul class='button'>
        <li><input type="button" id="exportButton" name="exportButton" onclick="submitForm('exportForm', '<%= charset %>', new Array('exportButton'))" value="导出"/></li>
      </ul>
    </form>
